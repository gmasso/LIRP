...
\begin{algorithm}
	...
	\begin{algorithmic}[1]
		...
		\STATE $LocationSampling(\ro,ns)$ \COMMENT{partitions $\ro$ into at most $ns$ subset of routes, regrouped by starting DC}
		\STATE $RouteSampling(\ro,ns)$	\COMMENT{partition $\ro$ into $ns$ independent subsets of routes}
		\FOR{$s=1$ to $ns$}
...
\OP{Pourquoi est-ce important d'allouer les clients restant à un dépot fictif ? Que fait-on si on n'a pas sélectionné $p$ dépots ? Et d'ailleurs comment la valeur de $p$ est elle choisie ? }

\OP{Est ce que c'est important de garder en mémoire les allocations ? Pour moi non, on n'a pas besoin de la matrice A'. }

\OP{Eviter d'appeler cet algorihtm une seule fois dans le préprocessing. Si c'est le cas, les choix opérés ici ne seront jamais remis en question. Il vaut mieux faire une sélection des dépots dans le cadre d'ue itération normale de l'algo 1. A chaque itération, on aura une sléection de dépots différente, et donc des sous-ensembles $\loc_i$ différents et des routes elite différentes }
...
as well as routes visiting clients that are not allocating to the starting DC \OP{Ca c'est valable uniquement si on sauvegarde l'affectation des clients aux DCs}. 
